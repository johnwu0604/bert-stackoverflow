...
class MultiLabelTextProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        data_df = pd.read_csv(os.path.join(data_dir, 'valid.csv'))
    ...
    def _create_examples(self, df, set_type):
        ...
        for (i, row) in enumerate(df.values):
            ...
            if set_type == 'test':
                ...
                for item in classes:
                    labels.append(1 if item in tags else 0)
            examples.append(
...
def serving_input_fn_builder(label_list):
    ...
    def serving_input_fn():
        label_ids = tf.placeholder(tf.int32, [len(label_list)], name='label_ids')
...
def main(_):
    ...
    if FLAGS.do_export:
        ...
        serving_input_fn = serving_input_fn_builder(label_list=label_list)
